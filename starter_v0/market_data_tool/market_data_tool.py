"""Tool tra cứu dữ liệu thị trường công khai, có giao diện gần giống yfinance.

Không cần thư viện ngoài. Dữ liệu được đọc từ Yahoo Finance public endpoints;
chỉ dùng cho mục đích tham khảo, không phải tư vấn đầu tư.
"""
...
class MarketDataError(RuntimeError):
    """Lỗi nghiệp vụ trả về cho agent thay vì làm crash tiến trình."""
...
def _request_json(path: str, params: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        raise MarketDataError(f"Yahoo Finance trả HTTP {exc.code} cho yêu cầu này.") from exc
    except URLError as exc:
        raise MarketDataError(f"Không kết nối được nguồn dữ liệu thị trường: {exc.reason}") from exc
    except (TimeoutError, json.JSONDecodeError) as exc:
        raise MarketDataError(f"Nguồn dữ liệu trả về không hợp lệ hoặc quá thời gian: {exc}") from exc


def _chart(symbol: str, range_: str, interval: str) -> dict[str, Any]:
    symbol = symbol.strip().upper()
    if not symbol:
        raise MarketDataError("symbol là bắt buộc, ví dụ AAPL, MSFT, BTC-USD hoặc VNM.VN.")
    if range_ not in VALID_RANGES:
        raise MarketDataError(f"range không hợp lệ: {range_}. Giá trị hợp lệ: {', '.join(sorted(VALID_RANGES))}.")
    if interval not in VALID_INTERVALS:
        raise MarketDataError(f"interval không hợp lệ: {interval}. Giá trị hợp lệ: {', '.join(sorted(VALID_INTERVALS))}.")
    ...
    if not result:
        ...
        raise MarketDataError(error.get("description") or f"Không tìm thấy mã {symbol}.")
    return result[0]


def quote_price(symbol: str) -> dict[str, Any]:
    """Trả giá gần nhất, tiền tệ, sàn và thời điểm cập nhật của một mã."""
    ...
    return {
        ...
        "disclaimer": "Dữ liệu chỉ để tham khảo, không phải tư vấn đầu tư.",
    }


def price_history(symbol: str, range_: str = "1mo", interval: str = "1d") -> dict[str, Any]:
    """Trả lịch sử OHLCV theo khoảng thời gian và tần suất, tương tự history() của yfinance."""
    ...
    for index, timestamp in enumerate(timestamps):
        ...
        for field, label in (("open", "open"), ("high", "high"), ("low", "low"), ("close", "close"), ("volume", "volume")):
...
def search_symbols(query: str, limit: int = 8) -> dict[str, Any]:
    """Tìm mã cổ phiếu, ETF, crypto hoặc chỉ số theo tên/mã."""
    query = query.strip()
    if not query:
        raise MarketDataError("query là bắt buộc.")
...
def market_data(action: str, symbol: str = "", range_: str = "1mo", interval: str = "1d", query: str = "", limit: int = 8) -> dict[str, Any]:
    """Điểm vào duy nhất cho agent: quote, history hoặc search."""
    ...
    raise MarketDataError("action phải là quote, history hoặc search.")


def main() -> None:
    parser = argparse.ArgumentParser(description="Market data tool, tương tự một phần yfinance.")
    parser.add_argument("action", choices=["quote", "history", "search"])
    ...
    try:
        print(json.dumps(market_data(**vars(args)), ensure_ascii=False, indent=2))
